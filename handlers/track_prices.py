...
async def enter_and_add_pair(message: types.Message, gateway: Gateway,
        ...
        if user:
            if new_pair in user.crypto_pairs:
                return await message.answer(
                    'The pair is tracking at the moment!')
            ...
            user = await gateway.user.create(message.chat.id,
                                             {new_pair: price})

        pairs_str = ', '.join(user.crypto_pairs)
        await message.answer(
            f'Tracking of this cryptocurrency pair has successfully started!\n'
            f'<b>{new_pair}</b>: {float(price):g}\n\n'
            f'<b>Current tracking pairs:</b>\n'
            f'<i>{pairs_str if pairs_str else "-"}</i>',
            reply_markup=inline_kb_close,
            parse_mode=types.ParseMode.HTML)
    await state.finish()


async def enter_and_remove_pair(message: types.Message, gateway: Gateway,
                                state: FSMContext):
    new_pair = message.text.upper()
    user = await gateway.user.get_by_chat_id(message.chat.id)
    if user:
        try:
            ...
            return await message.answer(
                'This pair is not in your list of tracking pairs!')
        except Exception as e:
            logging.error(
                f'Error: {type(e).__name__}, file: {__file__},'
                f' line: {e.__traceback__.tb_lineno}')
...
async def change_percent(callback_query: types.CallbackQuery):
    await callback_query.message.edit_text(
        'Enter percentage change in tracking:',
        reply_markup=inline_kb_back)
    await TrackPairs.change_percent.set()


async def enter_percent(message: types.Message, gateway: Gateway,
                        state: FSMContext):
    try:
...
async def back_to_track_prices(callback_query: types.CallbackQuery,
                               gateway: Gateway, state: FSMContext):
    await state.finish()
...
def register_track_prices(dp: Dispatcher):
    # message handlers
    dp.register_message_handler(enter_and_add_pair, state=TrackPairs.add_pair,
                                content_types=types.ContentTypes.TEXT)
    dp.register_message_handler(enter_and_remove_pair,
                                state=TrackPairs.remove_pair,
                                content_types=types.ContentTypes.TEXT)
    dp.register_message_handler(enter_percent, state=TrackPairs.change_percent,
                                content_types=types.ContentTypes.TEXT)
    ...
    dp.register_callback_query_handler(back_to_track_prices, Text('back'),
                                       state='*')
