...
async def check_prices(message: types.Message, gateway: Gateway,
    ...
    pairs = await get_price_of_pairs(client_session, list(user.crypto_pairs))
    if not pairs:
    ...
    if not message_text:
        return await message.answer(
            ...
            'please try again later or redefine cryptocurrency pairs.')

    await message.answer(message_text, reply_markup=inline_kb_close,
